...
foreach var in mean_growth_rate_2020_2022 {
	eststo clear
	forv i = 1/6{
		if `i' == 6{
...
(m6, keep(${dem2019}) rename(${dem2019} = "All IVs") mcolor(edkblue) ciopts(lcolor(edkblue)) mlabcolor(edkblue) mlabposition(12) ) ///
(r6,  keep(${dem2019}) rename(${dem2019} = "All IVs") offset(-0.2)  mlabposition(7)   mcolor(gs10) ciopts(lcolor(gs10)) mlabcolor(gs10)) ///
, xline(0, lcolor(red) lpattern(solid)) graphregion(color(white)) mlabel format(%9.2g) xtitle("Mean GDP Growth Rate in 2020-2022", box bcolor(red*0.3)) xscale(range(-5 5)) xlabel(-5(1)5) legend(order(2 "2SLS" 4 "OLS" 1 "95% CI") row(1) position(6))
