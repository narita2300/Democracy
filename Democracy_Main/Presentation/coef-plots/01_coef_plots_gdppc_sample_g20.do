...
gen g20 = 0
replace g20 = 1 if countries == "France" | countries =="United States" | countries == "Germany" | countries == "Japan" | countries == "United Kingdom" | countries == "Canada" | countries == "Italy" | countries == "Argentina" | countries == "Australia" | countries == "Brazil" | countries == "Canada" | countries == "China" | countries == "India" | countries == "South Korea" | countries == "Mexico" | countries == "Russia" | countries == "Saudi Arabia" | countries == "Turkey" | countries == "Indonesia"
...
, xline(0) graphregion(color(white)) mlabel format(%9.2g) xtitle("GDP Per Capita Growth Rate in 2020") xscale(range(-5 5)) xlabel(-5(1)5) title("Effect of 1 s.d. democracy increase on economic growth in 2020", span ) subtitle("Only G20, Base specification with no controls", span margin(b=5)) legend(order(2 "2SLS" 4 "OLS" 1 "95% CI"))

graph export ${path_presentation}/gdppc_coef_plot_g20.png, as(png) name("Graph") replace
...
, xline(0) graphregion(color(white)) mlabel mlabposition(12) mlabcolor(edkblue) format(%9.2g) xtitle("Mean GDP Per Capita Growth Rate in 2001-2019") xscale(range(-5 5)) xlabel(-5(1)5) title("Effect of 1 s.d. democracy increase on economic growth in 21st century", span size(medium)) subtitle("Base specification, no controls", span margin(b=5)) legend(order(2 "2SLS" 4 "OLS" 1 "95% CI"))

graph export ${path_presentation}/gdppc_21st_coef_plot.png, as(png) name("Graph") replace
...
(m6, keep(${dem2019}) rename(${dem2019} = "All IVs") mcolor(edkblue) ciopts(lcolor(edkblue)) mlabcolor(edkblue) mlabposition(12) ) ///
(r6,  keep(${dem2019}) rename(${dem2019} = "All IVs") offset(-0.2)  mlabposition(7)   mcolor(gs10) ciopts(lcolor(gs10)) mlabcolor(gs10)) ///
, xline(0) graphregion(color(white)) mlabel mlabposition(12) mlabcolor(edkblue) format(%9.2g) xtitle("Covid-19 Deaths Per Million in 2020") xscale(r(-100 800)) xlabel(0(200)800) title("Effect of 1 s.d. democracy increase on Covid-19 mortality in 2020", span size(medium)) subtitle("Base specification, no controls", span margin(b=5)) legend(order(2 "2SLS" 4 "OLS" 1 "95% CI"))

graph export ${path_presentation}/deaths_coef_plot_g20.png, as(png) name("Graph") replace
