...
foreach var in mean_growth_rate_2001_2019 gdp_growth2020 total_deaths_per_million {
	...
	if "`var'" == "total_deaths_per_million"{
		local last `" stats(N, fmt(0) labels("N")) prefoot(`" \hline \\[-1.8ex] IVs & \multicolumn{2}{c}{settler mortality} &  \multicolumn{2}{c}{population density} & \multicolumn{2}{c}{legal origin} & \multicolumn{2}{c}{language} & \multicolumn{2}{c}{crops \& minerals}  \\ Baseline Controls & \xmark & \cmark & \xmark & \cmark & \xmark & \cmark & \xmark & \cmark & \xmark & \cmark\\"') "'
	}
...
panelcombine, use(${path_appendix}/11_2sls_weighting_`var'_panel1.tex ${path_appendix}/11_2sls_weighting_`var'_panel2.tex ${path_appendix}/11_2sls_weighting_`var'_panel3.tex) paneltitles(" ") columncount(11) save(${path_appendix}/11_2sls_weighting_`var'.tex) cleanup
