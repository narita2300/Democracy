...
esttab using ${path_appendix}/4_first_stage_2000.tex, nocons replace label b(a1) se(a1) nostar nodepvars drop(${base_covariates2000} _cons) stats(N, labels("N")) obslast nogaps nonotes mlabels(none) posthead(`"\hline &\multicolumn{10}{c}{ Dependent Variable is Democracy Index (V-Dem, 2000)}\\\cline{2-11}\\[-1.8ex]"') prefoot(`"F-Statistic (First stage) & `fstat11' & `fstat12' & `fstat21' & `fstat22' & `fstat31' & `fstat32' & `fstat41' & `fstat42' & `fstat51' & `fstat52' \\ \hline \\[-1.8ex] Baseline Controls & \xmark & \cmark & \xmark & \cmark & \xmark & \cmark & \xmark & \cmark & \xmark & \cmark\\ "')
