...
if "`main_index'" == "csp" {
	local dem2019 "democracy_csp2018"
	local dem2000 "democracy_csp2000"
}

if "`main_index'" == "fh" {
	local dem2019 "democracy_fh2020"
	local dem2000 "democracy_fh2003"
}

if "`main_index'"== "vdem"{
	local dem2019 "democracy_vdem2019"
	local dem2000 "democracy_vdem2000"
}

if "`main_index'" == "eiu"{
	local dem2019 "democracy_eiu2020"
...
// replace legor_fr = . if civil_law==.
// replace legor_ge = . if civil_law==.
// replace legor_sc = . if civil_law==.
...
foreach remove_us_china in 0 1 {
	...
	forv j = 1/5{
		...
		eststo: ivregress 2sls `var' (`index'=${iv`j'}) `w', vce(robust)
		estat firststage
		mat fstat = r(singleresults)
		estadd scalar fs = fstat[1,4]
		
		eststo: ivregress 2sls `var' `covariates' (`index'=${iv`j'}) `w', vce(robust)
		estat firststage
		mat fstat = r(singleresults)
		estadd scalar fs = fstat[1,4]
		}
		
	// generate output by each outcome
	// di `num'
