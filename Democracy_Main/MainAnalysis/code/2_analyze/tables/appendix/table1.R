...
output <- stargazer(outcomes,
                    ...
                    covariate.labels = c(
                      ...
                      "Mean Nighttime Light Intensity Growth Rate in 2001-2013"),
                    column.sep.width = "0pt",
                    font.size= "small",
                    no.space = TRUE,
                    header = FALSE,
...
treatments <- total[, c("democracy_vdem2000", "democracy_vdem2019")]

# First, modify the treatments data directly
for (i in 1:nrow(treatments)) {
  for (j in 1:ncol(treatments)) {
    if (abs(treatments[i, j]) < 0.00001) {
      treatments[i, j] <- 0
    }
  }
}

# Now use stargazer with the modified data
output <- stargazer(treatments,
                    title = "Descriptive Statistics",
                    summary.stat = c("n", "mean", "sd", "min", "median","max"),
                    covariate.labels = c(
...
# Process the output and write to file
line = 11
i = 1
while (line < 11 + ncol(treatments)){
  print(i)
...
mechanisms <- total[, c(
  ...
  "mean_imp_2001_2019",
  "mean_exp_2001_2019",
  "new_pop_growth_2001_2019",
  "median_age_2001_2019",
  "primary_school1",
  "secondary_school1")]
...
output <- stargazer(mechanisms,
                    ...
                    summary.stat = c("n", "mean", "sd", "min", "median", "max"),
                    covariate.labels = c(
                      "Capital Investments (Mean Annual \\% Growth)",
                      "R\\&D Expenditure (Mean Annual \\% Growth)",
                      "Total Labor Force (Mean Annual \\% Growth)",
                      "TFP (Mean Annual \\% Growth)",
                      "Total Import Value (Mean Annual \\% Growth)",
                      "Total Export Value (Mean Annual \\% Growth)",
                      "Median Age (Mean Annual \\% Growth)",
                      "Population (Mean Annual \\% Growth)",
                      "Primary School Enrollment (Mean Annual \\% Growth)",
                      "Secondary School Enrollment (Mean Annual \\% Growth)"
                    ),
                    column.sep.width = "0pt",
                    font.size = "small",
                    no.space = TRUE,
