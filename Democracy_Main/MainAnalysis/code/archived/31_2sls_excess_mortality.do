...
eststo clear
forv i = 1/6{
		eststo: ivregress 2sls mean_deaths_pm2020_2022 /// Y: GDP growth rate in 2020
		...
		vce(robust) // robust 2SLS standard errors
 		...
 		eststo: ivregress 2sls mean_deaths_pm2020_2022 ${base_covariates2019} (${dem2019}=${iv`i'}) [w=total_gdp2019], vce(robust)
}

esttab using 17_2sls_excess_mortality.tex, ///
	replace label b(0) se(0) stats(N, labels("N")) nodepvars nogaps nonotes mlabels(none) keep(${dem2019}) ///
	posthead(`" \hline & \multicolumn{12}{c}{ Dependent Variable is Mean Excess Deaths Per Million in 2020-2022} \\ \cline{2-13}  \\[-1.8ex]"') ///
