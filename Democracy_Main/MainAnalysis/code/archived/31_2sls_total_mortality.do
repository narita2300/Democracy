...
eststo clear
forv i = 1/6{
		eststo: ivregress 2sls mean_excess_deaths_pt_who20_21  /// Y: Mean deaths per mil in 2020
		...
		vce(robust) // robust 2SLS standard errors
 		...
 		eststo: ivregress 2sls mean_excess_deaths_pt_who20_21 ${base_covariates2019} (${dem2019}=${iv`i'}) [w=total_gdp2019], vce(robust)
}

esttab using ${path_output}/appendix/17_2sls_mean_excess_mortality_20_21.tex, ///
	replace label b(0) se(0) stats(N, labels("N")) nodepvars nogaps nonotes mlabels(none) keep(${dem2019}) ///
	posthead(`" \hline & \multicolumn{12}{c}{ Dependent Variable is Mean Excess Deaths Per Thousand in 2020-2021} \\ \cline{2-13}  \\[-1.8ex]"') ///
