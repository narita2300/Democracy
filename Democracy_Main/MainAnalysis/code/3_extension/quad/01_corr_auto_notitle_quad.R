...
ggplot(total, 
  ...
  scale_fill_viridis(discrete=FALSE, guide=FALSE, option="A") +
  theme_ipsum(axis_title_size = 20) +
  ylab("Mean Night-time Light Intensity Growth Rate in 1992-2000") +
  xlab("Democracy Index (V-Dem) in 1990") +
  theme(legend.position = "none",
        axis.title.x = element_text(size = 14),
        axis.title.y = element_text(size = 14) ) + 
...
ggsave("figure_mean_g_night_light_1992_2000_quad.png", width=8, height=6)
...
# Mean Growth Rate Nighttime Light in 1992-2000 resid

total_complete <- total[!is.na(total$mean_g_night_light_1992_2000) & 
                          !is.na(total$abs_lat) &
                          !is.na(total$mean_temp_1981_1990) & 
                          !is.na(total$mean_precip_1981_1990) &
                          !is.na(total$median_age1990) &
...
reg <- lm(formula = mean_g_night_light_1992_2000 ~ abs_lat + mean_temp_1981_1990 + 
            mean_precip_1981_1990 + median_age1990 + pop_density1990, 
          weights = total_gdp1990, data = total_complete)
total_complete$mean_g_night_light_1992_2000_resid <- reg$residuals

ggplot(total_complete, 
       aes(x = democracy_vdem1990, 
           y = mean_g_night_light_1992_2000_resid,
           weight = total_gdp1990,
           size = total_gdp1990,
           fill = democracy_vdem1990)) + 
  geom_point(alpha = 0.5, 
...
ggsave("figure_mean_g_night_light_1992_2000_quad_resid.png", width=8, height=6)
