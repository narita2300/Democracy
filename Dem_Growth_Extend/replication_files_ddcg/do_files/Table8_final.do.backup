...
syntax anything[, ydeep(integer 1960) ystart(integer 1964) yfinal(integer $END_YEAR-1) truncate(integer 4) depvarlags(integer 4)]
	local 0 `anything' 
...
#delimit ;
estout e1A e2A e3A e4A e1B e2B e3B e4B   using ${project}/replication_files_ddcg/results/TableInterTop.tex, style(tex) 
...
#delimit ;
estout e1A_add e2A_add e3A_add e4A_add e1B_add e2B_add e3B_add e4B_add  using ${project}/replication_files_ddcg/results/TableInterTop_Add.tex, style(tex) 
...
#delimit ;
estout e1A e2A e3A e4A e1B e2B e3B e4B using ${project}/replication_files_ddcg/results/TableInterMid.tex, style(tex) 
varlabels(dem "Democracy" inter "Interaction") 
cells(b(star fmt(%9.3f)) se(par)) 
...
#delimit ;
estout e1A_add e2A_add e3A_add e4A_add e1B_add e2B_add e3B_add e4B_add  using ${project}/replication_files_ddcg/results/TableInterMid_Add.tex, style(tex) 
varlabels(longrun "Long-run effect of democracy"  effect$limit "Effect of democracy after $limit years"  persistence "Persistence of GDP process") 
cells(b(star fmt(%9.3f)) se(par)) 
keep(longrun effect$limit persistence) 
order(longrun effect$limit persistence)
...
bootstrap _b, seed(12345) reps(`repsBS') cluster(wbcode2) idcluster(newcl):  hhkBS y (dem inter L1.y L2.y L3.y L4.y) (y) (demreg intereg) (yy*), ydeep(1960) ystart(1964) yfinal($END_YEAR-1) truncate(4)
estimates store e1A
nlcom (shortrun: _b[dem])  (lag1: _b[L.y])  (lag2: _b[L2.y])  (lag3: _b[L3.y])  (lag4: _b[L4.y]), post
vareffects
estimates store e1A_add
...
#delimit ;
estout e1A e2A e3A e4A e1B e2B e3B e4B  using ${project}/replication_files_ddcg/results/TableInterBot.tex, style(tex) 
varlabels(dem "Democracy" inter "Interaction") 
cells(b(star fmt(%9.3f)) se(par)) 
...
#delimit ;
estout e1A_add e2A_add e3A_add e4A_add e1B_add e2B_add e3B_add e4B_add  using ${project}/replication_files_ddcg/results/TableInterBot_Add.tex, style(tex) 
varlabels(longrun "Long-run effect of democracy"  effect$limit "Effect of democracy after $limit years"  persistence "Persistence of GDP process") 
cells(b(star fmt(%9.3f)) se(par)) 
keep(longrun effect$limit persistence) 
order(longrun effect$limit persistence)
