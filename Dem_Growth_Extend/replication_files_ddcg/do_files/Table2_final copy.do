...
/*Sets base directory*/
/*********************/
// global project "/home/rg884/project/Dem_Growth_Extend"  /* Set base directory                         */
...
local repsBS=3                                                                            /* Number of bootstrap repetitions            */
...
program define hhkBS, eclass
local ydeep = $START_YEAR
local ystart = $START_YEAR + 4
local yfinal = $END_YEAR - 1
local truncate = 4
local depvarlags = 4
syntax anything[, ydeep(integer `ydeep') ystart(integer `ystart') yfinal(integer `yfinal') truncate(integer `truncate') depvarlags(integer `depvarlags')]
	local 0 `anything' 
...
file open myfile using ${project}/replication_files_ddcg/results/TableMain_lags.tex, write replace /*p-value for lags 5 to 8 not reported in the table*/
...
local ystart = $START_YEAR + 1
local yfinal = $END_YEAR - 1
bootstrap _b, seed(12345) reps(`repsBS') cluster(wbcode2) idcluster(newcl):  hhkBS y (dem L1.y) (y dem) () (yy*), ydeep($START_YEAR) ystart(`ystart') yfinal(`yfinal') truncate(4)
...
local ystart = $START_YEAR + 2
local yfinal = $END_YEAR - 1
bootstrap _b, seed(12345) reps(`repsBS') cluster(wbcode2) idcluster(newcl):  hhkBS y (dem L1.y L2.y) (y dem) () (yy*), ydeep($START_YEAR) ystart(`ystart') yfinal(`yfinal') truncate(4)
...
local ystart = $START_YEAR + 4
local yfinal = $END_YEAR - 1
bootstrap _b, seed(12345) reps(`repsBS') cluster(wbcode2) idcluster(newcl):  hhkBS y (dem L1.y L2.y L3.y L4.y) (y dem) () (yy*), ydeep($START_YEAR) ystart(`ystart') yfinal(`yfinal') truncate(4)
...
local ystart = $START_YEAR + 8
local yfinal = $END_YEAR - 1
bootstrap _b, seed(12345) reps(`repsBS') cluster(wbcode2) idcluster(newcl):  hhkBS y (dem L1.y L2.y L3.y L4.y L5.y L6.y L7.y L8.y) (y dem) () (yy*), ydeep($START_YEAR) ystart(`ystart') yfinal(`yfinal') truncate(4)
...
#delimit ;
estout e1 e2 e3 e4 e1gmm e2gmm e3gmm e4gmm e1md e2md e3md e4md  using ${project}/replication_files_ddcg/results/TableMain.tex, style(tex) 
...
stats(ar2p N N_g, fmt(%7.2f %7.0f  %7.0f) labels( "\input{results/TableMain_lags} \input{results/TableMain_Add}  AR2 test p-value" "Observations" "Countries in sample" ))  
...
#delimit ;
estout e1_add e2_add e3_add e4_add e1gmm_add e2gmm_add e3gmm_add e4gmm_add e1md_add e2md_add e3md_add e4md_add  using ${project}/replication_files_ddcg/results/TableMain_Add.tex, style(tex) 
